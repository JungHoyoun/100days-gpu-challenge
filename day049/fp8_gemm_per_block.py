...
def fp8_gemm_per_tensor(
    x_fp8, w_fp8, scale_x, scale_w, output,
    M, N, K,
    x_fp8_stride_m, x_fp8_stride_k,
    w_fp8_stride_k, w_fp8_stride_n,
    scale_x_stride_m, scale_x_stride_k,
    scale_w_stride_n, scale_w_stride_k,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        x_inv_s = tl.load(scale_x + offs_xm[:, None] * scale_x_stride_m + k * scale_x_stride_k)
        w_inv_s = tl.load(scale_w + pid_n * scale_w_stride_n + k * scale_w_stride_k)
        # We accumulate along the K dimension.
        accumulator += x_inv_s * w_inv_s * tl.dot(x, w)
...
def _to_float8(
    ...
    w_fp8_scaled_stride_n, w_fp8_scaled_stride_k,
    x_inv_s_stride_m, x_inv_s_stride_k,
    w_inv_s_stride_n, w_inv_s_stride_k,
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_N: tl.constexpr,
    BLOCK_SIZE_K: tl.constexpr
):
    ...
    w_ptrs = w + (offs_wn[:, None] * w_stride_n + offs_k[None, :] * w_stride_k)

    x = tl.load(x_ptrs, mask = masks_xm[:, None] * masks_k[None, :], other=0.0)
    w = tl.load(w_ptrs, mask = masks_wn[:, None] * masks_k[None, :], other=0.0)
    ...
    tl.store(
        w_inv_s + pid_n * w_inv_s_stride_n + pid_k * w_inv_s_stride_k,
        1 / w_scale
    )
    ...
    # w_fp8_scaled
    tl.store(
        w_fp8_scaled + (offs_wn[:, None] * w_fp8_scaled_stride_n + offs_k[None, :] * w_fp8_scaled_stride_k),
        w_scl_sat,
        mask = masks_wn[:, None] * masks_k[None, :]
    )
...
def custom_fp8gemm(x_fp8, w_fp8, out_dtype):
    ...
    x_fp8_scaled = torch.empty_like(x_fp8, dtype=torch.float8_e4m3fn)
    w_fp8_scaled = torch.empty_like(w_fp8, dtype=torch.float8_e4m3fn)

    x_inv_s = torch.zeros([M, num_pid_k], dtype=torch.float32, device="cuda")
    w_inv_s = torch.zeros([num_pid_n, num_pid_k], dtype=torch.float32, device="cuda")
    ...
    fp8_gemm_per_tensor[grid](
        x_fp8_scaled, w_fp8_scaled, x_inv_s, w_inv_s, output,
        M, N, K,
        x_fp8_scaled.stride(0), x_fp8_scaled.stride(1),
        w_fp8_scaled.stride(0), w_fp8_scaled.stride(1),
        x_inv_s.stride(0), x_inv_s.stride(1),
