...
class MoE(nn.Module):
    def __init__(self, config: Dict):
        ...
        # self.experts = nn.ModuleList(
        #     [torch.compile(Expert(config)) for _ in range(config["n_routed_experts"])]
        # )
        # self.gating_network = torch.compile(MoEGate(config))
        # shared_expert_dim = config["d_expert"] * config["n_shared_experts"]
        # self.shared_expert = torch.compile(
        #     Expert(config=config, d_expert=shared_expert_dim)
        # )
    ...
    def moe_infer(
        ...
        idxs = (
            flat_expert_indices.argsort()
        )  # expert 0번에 속하는 토큰 번호, 1번에 속하는 토큰 번호, ...
        counts = flat_expert_indices.bincount().cpu().numpy()  # 값이 몇 번 나오는지
        ...
        for expert_id, end_idx in enumerate(tokens_per_expert):
            ...
            expert_out.mul_(flat_expert_weights[idxs[start_idx:end_idx]])  # 여기부터
