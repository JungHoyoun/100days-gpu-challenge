...
def _to_float8(
    ...
    tl.store(
        w_inv_s + pid_k * w_inv_s_stride_k,
        1 / w_scale
    )
...
def custom_fp8gemm(x_fp8, w_fp8, out_dtype):
    ...
    w_inv_s = torch.empty([num_pid_n, num_pid_k], dtype=torch.float32, device="cuda")
    ...
    x_inv_s = x_inv_s.repeat_interleave(BLOCK_SIZE_K, dim=1)[:, :K]
    w_fp8_scaled * w_inv_s.repeat_interleave(BLOCK_SIZE_K, dim=1).repeat_interleave(BLOCK_SIZE_N, dim=0)
    ...
    # w = x_fp8_scaled.float() * x_inv_s.unsqueeze(2).repeat(1, 1, 32).view(48, 64)  # (48, 2, 32)
