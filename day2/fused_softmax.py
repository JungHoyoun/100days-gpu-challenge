...
def naive_softmax(x):
    ...
    # reads MN, write M elements
    x_max = x.max(dim=1)[0]

    # read MN + M, subtraction is MN flops, write MN elements
    z = x - x_max[:, None]

    # read MN, write MN elements
    numerator = torch.exp(z)

    # read MN, sum is MN flops, write M elements
    denominator = numerator.sum(1)
    
    # read MN+M, devision is MN flops, MN elements
...
def _softmax_kernel(
    ...
    for row_idx in tl.range(PID, n_rows, row_step, num_stages=num_stages):
        ...
        row = tl.load(input_ptrs, mask=mask, other=float('-inf')) # shape (BLOCK_SIZE) 
...
def benchmark(M, N, provider):
    ...
    if provider == 'torch_compile':
        ms = triton.testing.do_bench(lambda: softmax_torch_compile(x))
    gbps = lambda ms: 2 * x.numel() * x.element_size() * 1e-9 / (ms * 1e-3)
    return gbps(ms)

if __name__=="__main__":
    # test_softmax_kernel(size=(1823, 781))
    # test_softmax_kernel(size=(4097, 4646))
    # test_softmax_kernel(size=(9842, 13482))
