...
def _swiglu_backward_kernel(
    dLdy_ptr, dLdx1_ptr, dLdx2_ptr,
    x1_ptr, x2_ptr, x2x1sigmax1_ptr,
    n_elements,
    BLOCK_SIZE: tl.constexpr
):  
    PID = tl.program_id(0)
    ...
    dLdy = tl.load(dLdy_ptr + idx, mask=mask, other=.0)
    
    x1 = tl.load(x1_ptr + idx, mask=mask, other=.0)
    x2 = tl.load(x2_ptr + idx, mask=mask, other=.0)
    x2x1sigmax1 = tl.load(x2x1sigmax1_ptr + idx, mask=mask, other=.0)

    dLdx1 = dLdy * (x2 * tl.sigmoid(x1) + x2x1sigmax1 * (1 - tl.sigmoid(x1)))
    dLdx2 = dLdy * (x1 * tl.sigmoid(x1))

    tl.store(dLdx1_ptr + idx, dLdx1, mask=mask)
    tl.store(dLdx2_ptr + idx, dLdx2, mask=mask)
...
class Swiglu(torch.autograd.Function):
    ...
    def backward(ctx, dLdy):
        x1, x2, x2x1sigmax1 = ctx.saved_tensors
        n_elements = ctx.n_elements

        dLdx1 = torch.empty_like(x1, dtype=x1.dtype, device=x1.device)
        dLdx2 = torch.empty_like(x1, dtype=x1.dtype, device=x1.device)

        grid = lambda meta: (
            tl.cdiv(n_elements, ctx.BLOCK_SIZE),
        )
